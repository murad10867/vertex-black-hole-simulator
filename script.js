(() => {
  ...
  function addRandomBody() {
    ...
    addBody(cx + Math.cos(angle) * radius, cy + Math.sin(angle) * radius);
  }
  ...
  function drawBlackHole() {
    const lens = ctx.createRadialGradient(cx,cy,EVENT_HORIZON*.4,cx,cy,EVENT_HORIZON*2.3);
    ...
    ctx.fillStyle = lens;
    ctx.beginPath();
    ctx.arc(cx,cy,EVENT_HORIZON*2.3,0,Math.PI*2);
    ctx.fill();
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(cx,cy,EVENT_HORIZON+5,0,Math.PI*2);
    ctx.stroke();
  }
  ...
  function draw() {
    ...
    ctx.fillText('BLACK HOLE', cx, cy + EVENT_HORIZON + 32);
  }
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    if (Math.hypot(x-cx,y-cy) > EVENT_HORIZON + 35) {
      addBody(x,y);
    }
  });
